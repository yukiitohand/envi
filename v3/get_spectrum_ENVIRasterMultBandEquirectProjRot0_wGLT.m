function [spc,wv,band_idxes] = get_spectrum_ENVIRasterMultBandEquirectProjRot0_wGLT(...
    rastermb_wglt,s,l,varargin)
% [spc,wv,band_idxes] = get_spectrum_ENVIRasterMultBandEquirectProjRot0_wGLT(...
%     rastermb_wglt,s,l,varargin)
%  Get spectrum/spectra and corresponding wavelength from
%  ENVIRasterMultBandEquirectProjRot0_wGLT class object.
% INPUTS:
%   rastermb_wglt: ENVIRasterMultBandEquirectProjRot0_wGLT obj
...
if isempty(bands)
    bands_bool = true(rastermb_wglt.hdr.bands,1);
elseif ~islogical(bands)
    bands_bool = false(rastermb_wglt.hdr.bands,1);
...
if isfield(rastermb_wglt.hdr,'wavelength') && ...
        ~isempty(rastermb_wglt.hdr.wavelength)
    wv = rastermb_wglt.hdr.wavelength(:);
else
...
wdw_end = min(floor(ave_window/2)+[l,s], ...
    [rastermb_wglt.hdr.lines,rastermb_wglt.hdr.samples]);


line_idxes = wdw_strt(1):wdw_end(1);
smpl_idxes = wdw_strt(2):wdw_end(2);
samplesc = length(smpl_idxes); linesc = length(line_idxes);
spc = nan(linesc,samplesc,rastermb_wglt.hdr.bands);

if isempty(rastermb_wglt.RasterSource.img)
    for yi=1:linesc
        for xi=1:samplesc
            x=smpl_idxes(xi); y=line_idxes(yi);
            [spc(yi,xi,:),xf,yf] = rastermb_wglt.lazyEnviRead(x,y, ...
                'precision',precision,...
    ...
    for yi=1:linesc
        for xi=1:samplesc
            x=smpl_idxes(xi); y=line_idxes(yi);
            xf = rastermb_wglt.GLTdata.img(y,x,1);
            yf = rastermb_wglt.GLTdata.img(y,x,2);
            spc(yi,xi,:) = rastermb_wglt.RasterSource.img(yf,xf,:);
        end
    end
    if rastermb_wglt.RasterSource.is_img_band_inverse
        spc = flip(spc,3);
    end
end
...
elseif l_coeff==rastermb_wglt.hdr.bands
    spc = spc .* permute(coeff,[3,2,1]);
    % perform bands option if given
    spc = spc(:,:,bands_bool);
elseif l_coeff==1
