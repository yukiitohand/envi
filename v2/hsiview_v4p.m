function [  ] = hsiview_v4p( rgb,GLTdata_ar,hsiar,legends,varargin )
...
set(hdt,'UpdateFcn',{@map_cursor_crismx,rgb,GLTdata_ar,ax_spc,hsiar,legends,keepHdl,...
...
function output_txt = map_cursor_crismx(obj,event_obj,CData,GLTdata_ar,ax_spc,hsiar,legends,...
...
%     yyaxis(ax_spc,'right');
    for n=1:size(keepHdl.crd,1)
        ...
        for i=1:nhsi
            xfl = abs(GLTdata_ar{i}.img(l,s,1));
            yfl = abs(GLTdata_ar{i}.img(l,s,2));
            bands_i = bands{i};
            is_bands_inverse_i = is_bands_inverse(i);
            ave_window_i = ave_window{i};
            spc_shift = spc_shifts(i);
            varargin_plot_i = varargin_plot{i};
            [spc,wv] = hsiar{i}.get_spectrum(xfl,yfl,'BANDS',bands_i,...
                'BANDS_INVERSE',is_bands_inverse_i,'AVERAGE_WINDOW',ave_window_i);
            spc = spc+spc_shift;
            plot(ax_spc,wv,spc,varargin_plot_i{:},...
                 'DisplayName',sprintf('%s X:% 4d, Y:% 4d, XF:% 3d, YF:% 3d',...
                 legends{i},s,l,xfl,yfl));
            hold(ax_spc,'on');
            if ~isempty(hsiar{i}.BP1nan)
                [spcbp,wv] = hsiar{i}.get_spectrum(xfl,yfl,...
                    'BANDS',bands_i,'BANDS_INVERSE',is_bands_inverse_i,...
                    'AVERAGE_WINDOW',ave_window_i,...
                    'COEFF',hsiar{i}.BP1nan(:,xfl),'COEFF_INVERSE',hsiar{i}.is_bp1nan_inverse);
                spcbp = spcbp+spc_shift;
                plot(ax_spc,wv,spcbp,'x-',...
                        'DisplayName',sprintf('BP - %s X:% 4d, Y:% 4d, XF:% 3d, YF:% 3d',...
                        legends{i},s,l,xfl,yfl));
            end
            if ~isempty(hsiar{i}.GP1nan)
                [spcgp,wv] = hsiar{i}.get_spectrum(xfl,yfl,...
                    'BANDS',bands_i,'BANDS_INVERSE',is_bands_inverse_i,...
                    'AVERAGE_WINDOW',ave_window_i,...
                    'COEFF',hsiar{i}.GP1nan(:,xfl),'COEFF_INVERSE',hsiar{i}.is_gp1nan_inverse);
                spcgp = spcgp+spc_shift;
                plot(ax_spc,wv,spcgp,'+-',...
                        'DisplayName',sprintf('GP - %s X:% 4d, Y:% 4d, XF:% 3d, YF:% 3d',...
                        legends{i},s,l,xfl,yfl));
            end
...
s = pos(1); l = pos(2);
% xfl = abs(GLTdata.img(l,s,1));
% yfl = abs(GLTdata.img(l,s,2));
for i=1:nhsi
    xfl = abs(GLTdata_ar{i}.img(l,s,1));
    yfl = abs(GLTdata_ar{i}.img(l,s,2));
    bands_i = bands{i};
