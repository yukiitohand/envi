...
if Nin == 1
    if any(strcmpi(metainfo_image_input(1).class,[numeric_types,{'logical'}]))
        % input is just cdata, inside a cell
        i_cdata = 1; i_xdata = []; i_ydata = []; i_zdata = [];
        i_image_varargin = [];
    else
        i_cdata = []; i_xdata = []; i_ydata = []; i_zdata = [];
        i_image_varargin = [];
    end
elseif Nin > 1
    ...
    elseif (any(strcmpi(metainfo_image_input(1).class,numeric_types)) && any(metainfo_image_input(1).size==1)) ...
            ...
            && any(strcmpi(metainfo_image_input(1).class,[numeric_types,{'logical'}]))
        % if input is (x,y,C)
        i_xdata = 1; i_ydata = 2; i_cdata = 3; i_zdata = [];
        i_image_varargin = 4:Nin;
    else
        i_image_varargin = 1:Nin;
        i_xdata = []; i_ydata = []; i_cdata = []; i_zdata = [];
        for ii=1:2:length(metainfo_image_input)
            if ischar(metainfo_image_input(ii).char)
                switch upper(metainfo_image_input(ii).char)
                    case 'XDATA'
                        i_xdata =ii+1;
                    case 'YDATA'
                        i_ydata = ii+1;
