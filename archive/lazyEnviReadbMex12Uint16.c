/*
 * lazyEnviReadbMex12Uint16.c
 * Read a specified band of hyperspectral image data with data_type 12
 ...
 * 
 * Output:
 * im: (Line x sample) Matrix of the band of the image in double format
...
/* check the data_type is corret */
    if(data_type!=12)
    {   
        mexErrMsgIdAndTxt( "MATLAB:lazyEnviReadbMex12Uint16:data_type",
...
/* read the image file */
    ...
    imb = mxGetPr(plhs[0]);
    buf = (unsigned short *)malloc(s*samples);
    fp = fopen(fpath,"rb");
